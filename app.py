...
def evaluate_code():
    ...
    # Check if code is just the default boilerplate (no real solution written)
    boilerplate_indicators = [
        'def solve_problem():\n',
        'def solve_problem():\r\n',
    ]
    stripped_code = user_code.strip()
    
    # Detect if user hasn't written any real code
    is_boilerplate = False
    if any(indicator in user_code for indicator in boilerplate_indicators):
        # Remove comments and whitespace to check if there's actual logic
        code_lines = [line.strip() for line in stripped_code.split('\n') 
                     if line.strip() and not line.strip().startswith('#') and not line.strip().startswith('//')]
        # Filter out common boilerplate lines
        real_code_lines = [line for line in code_lines 
                          if line not in ('pass', 'def solve_problem():', 
                                        'if __name__ == "__main__":', 
                                        'print("Solution ready!")')]
        if len(real_code_lines) == 0:
            is_boilerplate = True
    ...
    if is_boilerplate:
        zero_evaluation = {
            ...
            'test_cases_passed': 0,
            'total_test_cases': 5,
            'detailed_feedback': 'No solution code was written. Please implement your solution before submitting.',
            'suggested_improvements': 'Write actual code that solves the given problem.',
            'time_complexity': 'N/A',
            'space_complexity': 'N/A'
        }
        
        # Save coding test with zero scores
        test_data = {
            'session_id': session.get('session_id'),
            ...
            'clarity_score': 0,
            'logic_score': 0,
            'feedback': zero_evaluation['detailed_feedback'],
        ...
        return jsonify({
            ...
            'execution': {
                'output': '',
                'error': 'No solution code written.',
                'success': False
            }
        })
    ...
    # AI evaluation — pass execution results so AI knows if code ran
    evaluation = ai_processor.evaluate_code(
        problem_statement=problem.get('problem_statement', ''),
        user_code=user_code,
        execution_success=success,
        execution_error=error if not success else None
    )
